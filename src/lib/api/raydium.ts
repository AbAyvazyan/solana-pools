...
const RAYDIUM_API_URL = process.env.NEXT_PUBLIC_RAYDIUM_API_URL || 'https://api-v3.raydium.io'
const RAYDIUM_PRICE_ENDPOINT = process.env.NEXT_PUBLIC_RAYDIUM_PRICE_ENDPOINT || '/mint/price'
const COINGECKO_API_URL = process.env.NEXT_PUBLIC_COINGECKO_API_URL || 'https://api.coingecko.com/api/v3'
const COINGECKO_SIMPLE_PRICE_ENDPOINT = process.env.NEXT_PUBLIC_COINGECKO_SIMPLE_PRICE_ENDPOINT || '/simple/price'
...
async function getTokenPriceFromRaydium(mintAddress: string): Promise<number | null> {
  try {
    logger.debug(`Fetching Raydium price for mint: ${mintAddress}`)
    
    const response = await fetch(`${RAYDIUM_API_URL}${RAYDIUM_PRICE_ENDPOINT}?mints=${mintAddress}`)
    
    logger.debug(`Raydium API Response Status: ${response.status} ${response.statusText}`)
    ...
    if (isNaN(price)) {
      logger.warn(`Invalid price data for mint: ${mintAddress}`, { price: data.data[mintAddress] })
      return null
    }
    ...
    logger.error('Error fetching price from Raydium', { mintAddress, error: error instanceof Error ? error.message : error })
    return null
  }
}

async function getTokenDataFromCoinGecko(symbol: string): Promise<RaydiumTokenData | null> {
  try {
    ...
    logger.error('Error fetching token data from CoinGecko', { symbol, error: error instanceof Error ? error.message : error })
    return null
  }
}

export async function getTokenData(symbol: string): Promise<RaydiumTokenData | null> {
  try {
    ...
    if (mintAddress) {
      ...
      if (raydiumPrice !== null) {
        logger.info('Raydium API successful, trying CoinGecko for additional data')
    ...
    if (coinGeckoData) {
      logger.info('CoinGecko fallback successful', { symbol, price: coinGeckoData.price })
    ...
    logger.error('Error fetching token data', { symbol, error: error instanceof Error ? error.message : error })
    return null
  }
}

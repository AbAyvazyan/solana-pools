...
const COINGECKO_TRENDING_ENDPOINT = process.env.NEXT_PUBLIC_COINGECKO_TRENDING_ENDPOINT || '/search/trending'
...
export async function getTrendingTokens(): Promise<CoinGeckoTrendingToken[]> {
  try {
    ...
    const response = await fetch(`${COINGECKO_API_URL}${COINGECKO_TRENDING_ENDPOINT}`)
    ...
    logger.error('Error fetching trending tokens', { error: error instanceof Error ? error.message : error })
    throw error
  }
}
...
export async function getTokenPrice(tokenId: string): Promise<number> {
  try {
    ...
    logger.error('Error fetching token price', { error: error instanceof Error ? error.message : error })
    return 0
  }
}

...
const JUPITER_TOKEN_LIST_URL =
  process.env.NEXT_PUBLIC_JUPITER_TOKEN_LIST_URL || 'https://token.jup.ag/all';
...
const CACHE_DURATION = parseInt(
  process.env.NEXT_PUBLIC_CACHE_DURATION || '300000'
); // 5 minutes
...
async function fetchTokenList(): Promise<JupiterToken[]> {
  ...
  try {
    ...
    logger.error('Error fetching Jupiter token list', {
...
export async function findMintAddress(
  ...
  try {
    ...
    if (token) {
      logger.debug(`Found mint address for ${symbol}`, {
    ...
    logger.error('Error finding mint address', {
      symbol,
      error: error instanceof Error ? error.message : error,
    });
    return undefined;
...
async function getTokenFromJupiter(
  symbol: string
  ...
  try {
    ...
    logger.error('Error getting token from Jupiter', {
      symbol,
      error: error instanceof Error ? error.message : error,
    });
    return undefined;
...
export async function getTokenMetadata(
  ...
  try {
    ...
    if (jupiterToken) {
      logger.info(`Found token in Jupiter`, {
      ...
      try {
        ...
        logger.warn('RPC failed, using Jupiter data without supply', {
          symbol,

  ...
  try {
    ...
    try {
      ...
      if (trendingToken) {
        ...
        logger.info('Found token in trending data with CoinGecko ID', {
          symbol,
          coinGeckoId,
        });
      }
    } catch (error) {
      logger.warn('Could not fetch trending data', {
        symbol,
    ...
    if (trendingTokenData?.data) {
      ...
      finalMarketCap = trendingTokenData.data.market_cap
        ? parseFloat(trendingTokenData.data.market_cap.replace(/[$,]/g, ''))
        : finalMarketCap;
      finalVolume24h = trendingTokenData.data.total_volume
        ? parseFloat(trendingTokenData.data.total_volume.replace(/[$,]/g, ''))
        : finalVolume24h;
      finalPriceChangePercent24h =
        trendingTokenData.data.price_change_percentage_24h?.usd ||
        finalPriceChangePercent24h;
    }
    ...
    const tokenData = {
      ...
      name:
        trendingTokenData?.name || solanaMetadata?.name || symbol.toUpperCase(),
      symbol:
        trendingTokenData?.symbol ||
        solanaMetadata?.symbol ||
        symbol.toUpperCase(),
      price: finalPrice,

  ...
  useEffect(() => {
    ...
    relatedTokens.forEach(relatedSymbol => {
      if (relatedSymbol !== symbol.toLowerCase()) {
        ...
        const cachedData = queryClient.getQueryData(['token-data', relatedSymbol])
        if (!cachedData) {
            ...
            <Card>
              <CardContent className="flex items-center justify-center py-8 text-red-600">
                <div className="text-center">
                  <h3 className="text-lg font-semibold mb-2">Error loading token data</h3>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-500">{(token.symbol || 'UNKNOWN').toUpperCase()}</p>
                </div>
              </div>
            </div>
            <Badge variant="secondary">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-gray-500 mt-2">
                24h change: {isPricePositive ? '+' : ''}{(token.priceChange24h || 0).toFixed(4)} USD
              </p>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatVolume(token.volume24h)}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatSupply(token.solana?.supply || 0)}</div>
                <p className="text-sm text-gray-500">{token.solana?.decimals || 0} decimals</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-sm font-mono break-all">{token.id || 'N/A'}</div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-500">Token Symbol</label>
                  <p className="text-lg">{token.solana?.symbol || 'UNKNOWN'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Total Supply</label>
                  <p className="text-lg">{formatSupply(token.solana?.supply || 0)}</p>
                </div>
                <div>
                ...
                {token.solana?.mintAddress && (
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium text-gray-500">Mint Address</label>
                    <p className="text-sm font-mono break-all">{token.solana.mintAddress}</p>
                  </div>
                )}
              </div>
            </CardContent>
